...
from ldsrs2_launcher.domain.raster import ensure_proj_env, parse_epsg, scale_to_uint16


LOGGER = logging.getLogger("ldsrs2")
...
def create_cube_with_retry(
    *,
    ...
    try:
        ...
        if config.retry_on_rate_limit and is_rate_limit_error(exc):
            raise RuntimeError("Sentinel-2 staging failed after retry policy was exhausted") from exc
        raise
